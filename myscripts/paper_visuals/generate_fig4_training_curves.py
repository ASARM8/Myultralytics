...
def load_tidy_series(path: Path) -> pd.DataFrame:
    ...
    output = frame[["epoch", "method", "map50_95"]].copy()
    output["source_kind"] = "checkpoint_validation"
    return output
...
def build_figure(frame: pd.DataFrame, smooth_window: int):
    ...
    for index, method in enumerate(methods):
        ...
        is_checkpoint_curve = (subset["source_kind"] == "checkpoint_validation").all()
        if is_checkpoint_curve:
            ax.plot(
                x,
                y,
                color=color,
            ...
            plotted = y
        elif smooth_window > 1 and len(y) >= smooth_window:
